...
final class MemorizeGameManagerView : UIView {
    ...
    private var gestureViews : [HandGestureView] = .init()
    ...
    private func showSequence() -> () {
        ...
        for gesture in self.handGestureSequenceBuffer {
            let view = HandGestureView(image: gesture.image)
    ...
    private func showStatus(isRightGesture : Bool, completion: (() -> ())? = nil) -> () {
        self.delegate?.shouldStopHandDetection()
    ...
    private func incrementIteration() -> () {
...
extension MemorizeGameManagerView : AttemptManagerViewDelegate {
